...
export class AIDebugWebviewProvider implements vscode.WebviewViewProvider {
    ...
    try {
      switch (message.command) {
        ...
        case 'submitContextForAnalysis':
          ...
          break;
        case 'openCopilotChatForAnalysis':
          await this.handleCopilotChatAnalysis();
          break;
  ...
  /**
   * Fallback method to open Copilot Chat with AI debug context
   * This bypasses the complex API submission and uses the native Copilot interface
   */
  private async openCopilotChatWithContext(contextFilePath: string): Promise<void> {
    try {
      console.log('Opening Copilot Chat with context from:', contextFilePath);
      
      // Read the context file
      const fs = require('fs');
      const contextContent = fs.readFileSync(contextFilePath, 'utf-8');
      
      // Prepare the analysis prompt with specific instructions for actionable fixes
      const analysisPrompt = `Please analyze this AI debug context and provide SPECIFIC, ACTIONABLE fixes for the failing tests:

${contextContent}

CRITICAL: For each fix, provide:
- 📁 **EXACT FILE PATH** (relative to project root)
- 📍 **LINE NUMBER** where the change should be made
...
- Identify the specific code changes causing test failures
- Point to exact files and lines where problems originated
- Explain how the changes broke existing functionality

### 2. 🛠️ CONCRETE FIXES (WITH FILE LOCATIONS)
...
CHANGE: 
  ...
  - AFTER:  new code here
REASON: Explanation of why this change fixes the issue
\`\`\`

### 3. 🧪 EXISTING TEST FIXES (WITH EXACT LOCATIONS)
For failing tests, specify:
- Test file path and line number
- Exact assertion that's failing
- Updated assertion or test setup needed
- Mock updates required

### 4. 🚀 IMPLEMENTATION ORDER
Rank fixes by priority with file references:
1. Fix core logic issues (specify files)
2. Update test expectations (specify test files) 
3. Fix any side effects (specify affected files)

### 5. 💡 VALIDATION STEPS
After each fix, explain how to verify it works:
- Which specific tests should pass
- Expected test output changes
- Commands to run for verification

**REMEMBER**: Every recommendation must include exact file paths and line numbers. No generic advice - only actionable, specific changes I can implement immediately!`;

      // Try multiple approaches to open Copilot Chat with auto-prompt
      try {
        // Method 1: Try the new Copilot Chat API (VS Code 1.85+)
        try {
          await vscode.commands.executeCommand('workbench.panel.chat.view.copilot.focus');
          await this.delay(500);
          
          // Try to send message directly to chat
          await vscode.commands.executeCommand('github.copilot.chat.sendMessage', analysisPrompt);
          
          this.sendMessage('analysisComplete', {
            success: true,
            message: 'Analysis context automatically sent to Copilot Chat. Check the Chat panel for AI analysis.',
            usedFallback: true
          });
          
          vscode.window.showInformationMessage('✅ Analysis context automatically sent to Copilot Chat! Check the Chat panel for AI analysis.');
          return;
          
        } catch (directSendError) {
          console.log('Direct message send failed, trying alternative...', directSendError);
        }
        
        // Method 2: Try using the chat participant API with @workspace
        try {
          const chatQuery = `@workspace ${analysisPrompt}`;
          ...
          this.sendMessage('analysisComplete', {
            success: true,
            message: 'Analysis context opened in Copilot Chat with prompt. Check the Chat panel.',
            usedFallback: true
          });
          
          vscode.window.showInformationMessage('✅ Analysis context opened in Copilot Chat with prompt! Check the Chat panel.');
          return;
          
        } catch (participantError) {
          console.log('Chat participant API failed, trying direct chat focus...', participantError);
        }
        
        // Method 2.5: Try simpler chat open with query
        try {
          await vscode.commands.executeCommand('workbench.action.chat.openInSidebar');
          await this.delay(800);
          
          // Try to insert text into chat input
          await vscode.commands.executeCommand('type', { text: analysisPrompt });
          
          this.sendMessage('analysisComplete', {
            success: true,
            message: 'Analysis context typed into Copilot Chat. Press Enter to send.',
            usedFallback: true
          });
          
          vscode.window.showInformationMessage('✅ Analysis context typed into Copilot Chat! Press Enter to send the analysis request.');
          return;
          
        } catch (typeError) {
          console.log('Type command failed, using clipboard method...', typeError);
        }
        
        // Method 3: Use clipboard + focus (most reliable fallback)
        await vscode.env.clipboard.writeText(analysisPrompt);
        await vscode.commands.executeCommand('workbench.panel.chat.view.copilot.focus');
        
        this.sendMessage('analysisComplete', {
          success: true,
          message: 'Analysis prompt copied to clipboard and Copilot Chat opened. Paste (Ctrl/Cmd+V) to analyze.',
          usedFallback: true
        });
        
        vscode.window.showInformationMessage('📋 Analysis prompt copied to clipboard and Copilot Chat opened! Paste (Ctrl/Cmd+V) to analyze.');
        
      } catch (commandError) {
        console.log('Copilot Chat command failed, trying alternative approach...', commandError);
        
        // Method 2: Open Copilot Chat via workbench action
        try {
          await vscode.commands.executeCommand('workbench.panel.chat.view.copilot.focus');
          
          this.sendMessage('analysisComplete', {
            success: true,
            message: 'Copilot Chat panel opened. Please paste the context manually for analysis.',
            usedFallback: true,
            contextContent: contextContent.substring(0, 1000) + '...' // Send truncated content for reference
          });
          
          vscode.window.showInformationMessage('📋 Copilot Chat opened! The context is available - please paste it into the chat for analysis.');
          
        } catch (panelError) {
          console.log('Panel focus failed, trying generic chat command...', panelError);
          
          // Method 3: Try generic chat command
          await vscode.commands.executeCommand('github.copilot.chat.open');
          
          // Copy context to clipboard as backup
          await vscode.env.clipboard.writeText(analysisPrompt);
          
          this.sendMessage('analysisComplete', {
            success: true,
            message: 'Copilot Chat opened and analysis prompt copied to clipboard. Paste in chat for analysis.',
            usedFallback: true
          });
          
          vscode.window.showInformationMessage('📋 Copilot Chat opened! Analysis prompt copied to clipboard - paste it in the chat.');
        }
      }
      
    } catch (error) {
      console.error('Failed to open Copilot Chat with context:', error);
      
      // Final fallback: just open the context file and show instructions
      try {
        const contextUri = vscode.Uri.file(contextFilePath);
        await vscode.window.showTextDocument(contextUri);
        
        const instruction = `To analyze this context with Copilot:
1. Select all content (Ctrl/Cmd+A)
2. Right-click and choose "Copilot: Explain This"
3. Or open Copilot Chat and ask it to analyze the selected text`;
        
        vscode.window.showInformationMessage(instruction, 'Open Copilot Chat').then(choice => {
        ...
        this.sendMessage('analysisComplete', {
          success: true,
          message: 'Context file opened. Use Copilot commands to analyze the content.',
          usedFallback: true
        });
        
      } catch (fileError) {
        console.error('Failed to open context file:', fileError);
        throw new Error('Failed to open Copilot Chat and context file. Please check that the Copilot extension is installed and active.');
      }
    }
  }
...
==========================================================`;
  ...
  private async handleContextSubmission(options?: SubmissionOptions): Promise<void> {
    try {
      ...
      let result;
      try {
        result = await this.copilotSubmissionService.submitContextForAnalysis(contextFilePath, options || {});
        console.log('Context submission completed successfully');
        // Send success message
        this.sendMessage('analysisComplete', result);
      } catch (apiError) {
        console.log('API submission failed, falling back to Copilot Chat panel...');
        // Fallback: Open Copilot Chat with context content
        await this.openCopilotChatWithContext(contextFilePath);
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('ENOENT') || error.message.includes('not found')) {
          ...
          // Try fallback to Copilot Chat if Copilot isn't available via API
          try {
            console.log('Copilot API not available, trying Copilot Chat fallback...');
            const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
            if (workspaceRoot) {
              const contextFilePath = path.join(workspaceRoot, '.github/instructions/ai_utilities_context/ai-debug-context.txt');
              await this.openCopilotChatWithContext(contextFilePath);
            ...
            console.error('Fallback to Copilot Chat also failed:', fallbackError);
          }
          errorMessage += '\n\n🔍 Troubleshooting:\n• Check GitHub Copilot extension status\n• Sign in to GitHub Copilot\n• Verify your Copilot subscription is active\n• Try using "Use Copilot Chat" option if available';
      ...
      // Also show VSCode error notification with fallback option
      const choice = await vscode.window.showErrorMessage(`Analysis failed: ${errorMessage}`, 'Use Copilot Chat', 'Cancel');
      if (choice === 'Use Copilot Chat') {
        try {
          ...
          console.error('Failed to open Copilot Chat:', chatError);
          vscode.window.showErrorMessage('Failed to open Copilot Chat');
        }
      }
    }
  }

  private async handleCopilotChatAnalysis(): Promise<void> {
    try {
      const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
      if (!workspaceRoot) {
        throw new Error('No workspace folder is open. Please open a project folder first.');
      ...
      if (!require('fs').existsSync(contextFilePath)) {
        vscode.window.showWarningMessage('AI debug context file not found. Please generate the context first by running the AI Debug workflow.');
        return;
      }
      ...
      console.error('Failed to open Copilot Chat for analysis:', error);
